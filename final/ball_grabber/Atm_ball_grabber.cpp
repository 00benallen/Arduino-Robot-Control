...
Atm_ball_grabber& Atm_ball_grabber::begin( int sensorPin ) {
  // clang-format off
  const static state_t state_table[] PROGMEM = {
    ...
    /*      HOLDING_ON */      ENT_HOLDING_ON,      LP_HOLDING_ON,      -1, WAITING_TO_GRAB,     IDLE,   RELEASING,         -1,   -1,
...
void Atm_ball_grabber::action( int id ) {
  switch ( id ) {
    ...
    case LP_WAITING_TO_GRAB:
      ballDetected = digitalRead(sensorPin) == LOW;
      return;
    case ENT_HOLDING_ON:
      claw.write(120);
